...
function roleMeta(role: ChatMessage["role"]) {
  switch (role) {
    case "user":      return { bubble: "bg-primary text-primary-foreground", align: "items-end" };
    case "assistant": return { bubble: "bg-card border border-border text-foreground", align: "items-start" };
    case "system":    return { bubble: "bg-muted/70 text-muted-foreground border border-border/40 text-xs", align: "items-start" };
    default:          return { bubble: "bg-muted text-foreground", align: "items-start" };
  }
}
    ...
    <div className="mt-3 overflow-hidden rounded-xl border border-primary/20 bg-background/80 text-foreground">
      ...
      <div className="divide-y divide-border/60">
        <div className="px-3 py-2">
          ...
          {preview.description && (
            <p className="mt-0.5 line-clamp-3 text-[11px] text-muted-foreground">{preview.description}</p>
          )}
        </div>
        <div className="flex flex-wrap items-center gap-2 px-3 py-1.5">
          ...
          {preview.deadline && (
            <span className="flex items-center gap-1 text-[10px] text-muted-foreground">
              ...
              {new Date(preview.deadline).toLocaleDateString("ru-RU", { day: "numeric", month: "short", year: "numeric" })}
            </span>
          )}
...
/* ── Кнопки proposal ── */
function ProposalCard({ orgId, proposal, actions, onDone }: {
  orgId: number; proposal: ChatProposal; actions: string[]; onDone: (msgs: ChatMessage[]) => void;
}) {
    ...
    onSuccess: (res) => {
      ...
      if (res.task) {
        ...
        toast.success(`Задача #${taskId} создана`, {
          action: {
            label: "Открыть",
            onClick: () => void navigate({ to: "/tasks/$taskId", params: { taskId: String(taskId) } }),
          },
        });
  ...
  if (done) {
    return (
      <div className="mt-2 flex flex-wrap items-center gap-2">
        {accept.isSuccess ? (
          <>
            <span className="text-xs text-emerald-600">✓ {taskId ? `Задача #${taskId} создана` : "Выполнено"}</span>
            {taskId && (
  ...
  return (
    <div className="mt-3 flex flex-wrap gap-2">
      {actions.includes("accept") && (
        <Button size="sm" disabled={busy} onClick={() => accept.mutate()} className="h-7 gap-1 text-xs">
          {accept.isPending ? <Loader2 className="h-3 w-3 animate-spin" /> : <CheckCircle2 className="h-3 w-3" />}
          Создать
        </Button>
      )}
      {(actions.includes("reject") || actions.includes("cancel")) && (
        <Button size="sm" variant="outline" disabled={busy} onClick={() => reject.mutate()} className="h-7 gap-1 text-xs">
...
/* ── Одно сообщение ── */
function MessageBubble({ msg, orgId, proposals, onNewMessages }: {
  msg: ChatMessage; orgId: number; proposals: ChatProposal[];
  ...
  return (
    <div className={cn("flex flex-col gap-0.5", align)}>
      {!isUser && msg.role !== "system" && (
        <p className="px-1 text-[10px] font-medium text-muted-foreground">{msg.author_name ?? "Ассистент"}</p>
      )}
      <div className={cn(
        ...
        {proposal && actions.length > 0 && (
          <ProposalCard orgId={orgId} proposal={proposal} actions={actions} onDone={onNewMessages} />
        )}
        {msg.meta?.task_id && msg.role === "system" && (
          <Link to="/tasks/$taskId" params={{ taskId: String(msg.meta.task_id) }}
...
function AutomationsPanel({ orgId }: { orgId: number }) {
  ...
  const { data: automations = [] } = useQuery({
    queryKey: ["automations", orgId], queryFn: () => orgApi.automations(orgId), staleTime: 30_000,
  });
  ...
  return (
    <div className="border-t border-border bg-card/50 px-4 py-2">
      ...
      <ul className="space-y-1">
        {active.map((a) => (
          <li key={a.id} className="flex items-center gap-2 rounded-xl bg-muted/40 px-2.5 py-1.5 text-xs">
            <span className={cn("shrink-0 rounded-full px-1.5 py-0.5 text-[10px] font-medium",
              STATUS_COLORS[a.status] ?? "bg-muted")}>{a.status}</span>
            <span className="min-w-0 flex-1 truncate text-foreground">{a.title}</span>
            {a.result_task_id && (
              <Link to="/tasks/$taskId" params={{ taskId: String(a.result_task_id) }}
            ...
            {a.schedule_cron && (
              <span className="hidden shrink-0 font-mono text-[10px] text-muted-foreground sm:block">{a.schedule_cron}</span>
            )}
...
/* ── Список чатов ── */
function ChatList({ orgId, activeChatId, onSelect }: {
  orgId: number; activeChatId: number | null; onSelect: (chat: OrgChat) => void;
}) {
  const { data: chats = [], isPending } = useQuery({
    queryKey: ["chats", orgId], queryFn: () => orgApi.chats(orgId), staleTime: 60_000,
  });
  if (isPending) return (
    <div className="space-y-1.5 p-2">
      {[0, 1].map((i) => <div key={i} className="h-11 animate-pulse rounded-xl bg-muted" />)}
    </div>
  );
  return (
        ...
        <li key={c.id}>
          <button type="button" onClick={() => onSelect(c)}
            className={cn(
              ...
              activeChatId === c.id ? "bg-primary/10 font-medium text-primary" : "text-foreground hover:bg-accent",
            )}>
            <div className={cn("flex h-7 w-7 shrink-0 items-center justify-center rounded-lg",
              c.type === "org" ? "bg-primary/15 text-primary" : "bg-accent text-accent-foreground")}>
              {c.type === "org" ? <MessageSquare className="h-3.5 w-3.5" /> : <Bot className="h-3.5 w-3.5" />}
            </div>
            <div className="min-w-0">
              <span className="block truncate">{c.title}</span>
              {c.bot_code && <span className="block truncate text-[10px] text-muted-foreground">{c.bot_code}</span>}
            </div>
          </button>
        </li>
      ))}
      {chats.length === 0 && <li className="py-6 text-center text-xs text-muted-foreground">Чатов пока нет</li>}
    </ul>
  );
}

/* ── Область сообщений ── */
function ChatWindow({ orgId, chat, canWrite }: { orgId: number; chat: OrgChat; canWrite: boolean }) {
  ...
  useEffect(() => { setLocalMsgs([]); setLocalProposals([]); }, [chat.id]);

  const allMsgs = [...serverMsgs];
  localMsgs.forEach((lm) => { if (!allMsgs.find((sm) => sm.id === lm.id)) allMsgs.push(lm); });
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex shrink-0 items-center gap-3 border-b border-border bg-card/40 px-4 py-2.5">
        <div className={cn("flex h-8 w-8 shrink-0 items-center justify-center rounded-xl",
          ...
          {chat.type === "org" ? <MessageSquare className="h-4 w-4" /> : <Bot className="h-4 w-4" />}
        </div>
        <div className="min-w-0 flex-1">
          <p className="truncate text-sm font-semibold text-foreground">{chat.title}</p>
          {chat.bot_code && <p className="truncate text-[10px] text-muted-foreground">{chat.bot_code}</p>}
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3">
        {historyPending ? (
          ...
          <div className="space-y-3">
            {allMsgs.map((msg) => (
              <MessageBubble key={msg.id} msg={msg} orgId={orgId} proposals={localProposals}
                onNewMessages={(newMsgs) => setLocalMsgs((prev) => {
                  const ids = new Set(prev.map((m) => m.id));
              ...
              <div className="flex flex-col items-start gap-0.5">
                ...
                <div className="rounded-2xl rounded-bl-sm border border-border bg-card px-4 py-3 shadow-sm">
                  <div className="flex items-center gap-1">
                    {[0, 1, 2].map((i) => (
                      <span key={i} className="h-2 w-2 animate-bounce rounded-full bg-muted-foreground/50"
        ...
        <div className="shrink-0 border-t border-border bg-card/60 px-3 py-2.5 backdrop-blur">
          <div className="flex items-end gap-2 rounded-2xl border border-border bg-background px-3 py-2 transition-all focus-within:border-primary/40 focus-within:ring-1 focus-within:ring-primary/10">
            <Textarea ref={textareaRef} value={text} onChange={(e) => setText(e.target.value)}
              placeholder="Напишите запрос ассистенту…" rows={1} disabled={send.isPending}
            ...
            <Button size="icon" className="h-8 w-8 shrink-0 rounded-xl"
              disabled={!text.trim() || send.isPending} onClick={handleSend}>
              {send.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <SendHorizontal className="h-4 w-4" />}
            </Button>
          </div>
          <p className="mt-1 px-1 text-[10px] text-muted-foreground">⌘+Enter — отправить · ответ 1–5 сек</p>
        </div>
      ) : (
...
function ChatPage() {
  ...
  useEffect(() => {
    ...
    if (chats.length === 0 && !openOrg.isPending) { openOrg.mutate(); }
    else if (chats.length > 0 && chats[0]) { setActiveChatId(chats[0].id); setMobileShowList(false); }
  ...
  return (
    <AppLayout>
      ...
      <div className="mb-3 flex items-center gap-2 md:hidden">
        <button type="button" onClick={() => setMobileShowList((v) => !v)}
        ...
        <div className="flex h-full">
          ...
          <aside className={cn(
            ...
            <div className="flex-1 overflow-y-auto">
              <ChatList orgId={org.id} activeChatId={activeChatId}
                onSelect={(c) => { setActiveChatId(c.id); setMobileShowList(false); }} />
            </div>
          </aside>
